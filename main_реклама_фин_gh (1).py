# ╔══════════════════════════════════════════════════════════════════╗
# ║  WB Реклама Финансы → Автоматизация_реклама_фин  |  v2.0        ║
# ║                                                                  ║
# ║  Шаг 0: /adv/v1/promotion/count  — диагностика                  ║
# ║  Шаг 1: /api/advert/v2/adverts   — список advertId + названия   ║
# ║  Шаг 2: /adv/v1/upd (БЕЗ id)    — один запрос, все транзакции  ║
# ║                                                                  ║
# ║  Реальные поля API:                                              ║
# ║    updTime      → Дата списания                                  ║
# ║    campName     → Кампания                                       ║
# ║    paymentType  → Источник списания (Баланс / Счёт / Бонусы…)   ║
# ║    updNum       → Номер документа                                ║
# ║    updSum       → Сумма                                          ║
# ║    advertId     → ID кампании                                    ║
# ║    advertType   → Раздел (9=Поиск, 8=АРК…)                      ║
# ║                                                                  ║
# ║  Фильтр: исключаем пополнения баланса (paymentType содержит     ║
# ║  «Пополнение» или updSum > 0 — т.е. зачисления)                 ║
# ╚══════════════════════════════════════════════════════════════════╝
import os, tempfile
...
# ════════════════════════════════════════════════════════════════════
#  HTTP  — та же схема что в main_реклама.py
# ════════════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════════════
#  ШАГ 0: ДИАГНОСТИКА  — точная копия из main_реклама.py
# ════════════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════════════
#  ШАГ 1: СПИСОК КАМПАНИЙ  — точная копия из main_реклама.py
...
#  /adv/v1/upd БЕЗ параметра id возвращает ВСЕ транзакции аккаунта
#  за период — один запрос вместо 33. Каждая транзакция содержит
#  advertId — по нему обогащаем данными из кампании.
#
#  Реальные поля (из диагностики):
#    updTime      — дата/время
#    campName     — название кампании (напрямую из API!)
#    paymentType  — источник: «Баланс», «Счёт», «Промо бонусы» и т.д.
#    updNum       — номер документа (еженедельный отчёт)
#    updSum       — сумма списания
#    advertId     — ID кампании
#    advertType   — тип: 9=Поиск, 8=АРК и т.д.
#    advertStatus — статус кампании
# ════════════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════════════
#  GOOGLE SHEETS  — та же схема что в main_реклама.py
# ════════════════════════════════════════════════════════════════════
